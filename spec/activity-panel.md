...
Если передан `expandedEntryId`, раскрытие становится controlled: панель отображает
раскрытой только совпадающую запись и сообщает намерение изменить выбор через
`onExpandedEntryChange`. Это позволяет приложению реализовать permalink/deep link,
не связывая React-слой с конкретным роутером.
